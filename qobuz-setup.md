...
To stream Qobuz music to Rasperry Pi or the Linux computer, you can use any UPnP/DLNA controller app on your smartphone or tablet. MConnectLive app(Android/iOs) was found to work the best.  When you open the app, you should see the Qobuz music library as a UPnP/DLNA server and the NPlay device (MyPiQoBuz in this example) as a playback device. Go to NPlay web interface and turn stream play on from the mrnu. If you play a track from the Qobuz library, it should start playing on NPlay with visualizations and DSP effects applied.
